...
#
from utils.windMouse import wind_mouse  # Импортируем функцию wind_mouse
...
def mouse_redirection(args, boxes):
    ...
    width = boxes[min_index, 2] - boxes[min_index, 0]
    last = destination
    destination = boxes_center[min_index].astype(int)

    # Используем WindMouse для плавного перемещения мыши к ближайшему боксу
    current_pos = win32api.GetCursorPos()  # Текущая позиция мыши
    wind_mouse(current_pos[0], current_pos[1], destination[0], destination[1])  # Плавное наведение
